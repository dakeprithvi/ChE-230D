...
## PY_PICKLE_SRC, are the python files that create .pickle files. The
...
# For making markup papers
PAPER_PREVIOUS := $(addprefix $(BUILD_DIR)/,$(ARTICLE_SRC:.tex=-previous.tex))
PAPER_DIFF_MARKUP := $(addprefix $(BUILD_DIR)/,$(ARTICLE_SRC:.tex=-markup.tex))
PAPER_BUILD_MARKUP := $(addprefix $(BUILD_DIR)/,$(ARTICLE_SRC:.tex=-markup.pdf))
PAPER_MARKUP := $(notdir $(PAPER_BUILD_MARKUP))
PAPER_MISC_MARKUP := $(foreach paper, $(basename $(PAPER_BUILD_MARKUP)), $(addprefix $(paper), $(TEXSUFFIXES)))
...
AUTODEPENDENCIES_SRC := $(PY_MAT_SRC) $(OCT_MAT_SRC) $(PY_PLOT) $(OCT_PLOT) \
                        ...
                        $(JL_MAT_SRC) $(GNUPLOT_SRC) $(PAPER_SRC) \
						$(MATLAB_MAT_SRC) $(PY_PICKLE_SRC)
...
CLEAN_FILES := \
	...
	$(PAPER_PREVIOUS) $(PAPER_DIFF_MARKUP) $(PAPER_BUILD_MARKUP) \
	$(PAPER_MARKUP) $(PAPER_MISC_MARKUP) \
...
## Copy pickle files to backup directory.
...
## Rules to make markup changes to an article based on previous commit
$(PAPER_PREVIOUS) : $(BUILD_DIR)/%-previous.tex : 
	@echo Checkout $@ from commit: $(COMMIT)
	@git show $(COMMIT):$*.tex > $@

$(PAPER_DIFF_MARKUP) : $(BUILD_DIR)/%-markup.tex : $(BUILD_DIR)/%-previous.tex %.tex
	@echo Running latexdiff on $(BUILD_DIR)/$*-previous.tex and $*.tex
	@latexdiff $(BUILD_DIR)/$*-previous.tex $*.tex > $@

$(PAPER_BUILD_MARKUP): $(BUILD_DIR)/%.pdf : $(BUILD_DIR)/%.tex
	@echo Making $@
	@./$(SCRIPT_DIR)/latex2pdf.py $(PDFLATEX_FLAGS) --dir $(BUILD_DIR) $(BUILD_DIR)/$*.tex

$(PAPER_MARKUP): %.pdf : $(BUILD_DIR)/%.pdf
	@echo "Copying $<"
	@cp $< $@
